...
public class Visual {
	...
	public void MenuCadastrarLivroP(Biblioteca library) {
		...
		GeneralBook nGnrlbk = new GeneralBook(name,
	...
	public void MenuEmprestimo(Biblioteca library, Calendar c) {
		...
		List<User> userSearch = library.getUsers().stream()
				.filter(s -> s.getName().equals(uName))
				.collect(Collectors.toList());
		System.out
				.println("Entre com o indice do usuario correspondente ao ID :");
		for (int i = 0; i < userSearch.size(); i++) {
			System.out.println("[" + i + "] Nome : "
					+ userSearch.get(i).getName() + " ID : "
					+ userSearch.get(i).getID());
		}
		...
		if (uPos > userSearch.size() || userSearch.get(uPos).getPenality() > 0) {
			...
			List<Book> bookSearch = library.getBooks().stream()
					.filter(s -> s.getName().equals(bName))
					.collect(Collectors.toList());
			System.out
					.println("Entre com o indice do livro correspondente ao ID :");
			for (int i = 0; i < bookSearch.size(); i++) {
				System.out.println("[" + i + "] Nome : "
						+ bookSearch.get(i).getName() + " ID : "
						+ bookSearch.get(i).getID());
			}
			int bPos = Integer.parseInt(in.nextLine());
			if (bPos > bookSearch.size()
					|| bookSearch.get(bPos).getStatus() == false) {
				System.out.println("Livro indisponivel ou entrada invalida");
			} else {

				Rent nRnt = new Rent(userSearch.get(uPos),
						bookSearch.get(bPos), c, library.getRents().size() + 1);
	...
	public void printUser(Biblioteca library, User us) {
		...
		System.out.println("Maximo de emprestimos : " + us.getMaxRents()
				+ " durante no máximo : " + us.getRentMaxDays());
